...
class SetFoodLevel(val plugin: God) : CommandExecutor {
        ...
        if (sender is Player) {
            if (sender.hasPermission("GodCraft.SetFoodLevel")) {
                ...
                if (args.size > 2) {
                    ...
                    if (level > plugin.config.getInt("maxfoodlevel", 20)) {
                        sender.sendMessage(ChatColor.RED.toString() + "That is to much. Please use a number between 0 and " + plugin.config.getInt("maxfoodlevel", 20))
                        return true
                    }
                }

                targetPlayer.foodLevel = level
                sender.sendMessage(ChatColor.YELLOW.toString() + "Food Level Set!")
                return true
            } else {
